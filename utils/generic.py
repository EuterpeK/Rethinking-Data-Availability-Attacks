...
def get_mix_loader(
    dataset, batch_size, root='./data', train=True, clean=0,
        noise_path=None, poisoned_indices_path=None, fitr=None):
    
    if dataset == 'imagenet' or dataset == 'imagenet-mini':
        return NotImplementedError("Imagenet is too large to train, it is a waste of time!")
    
    # if dataset == 'imagenet' or dataset == 'imagenet-mini':
    #     return imagenet_utils.get_poisoned_loader(
    #             dataset, batch_size, root, train, noise_path, poisoned_indices_path, fitr)

    target_set = get_dataset(dataset, root=root, train=train, fitr=fitr)
    data_nums = len(target_set)
    
    if poisoned_indices_path is None:
        raise FileExistsError("{}, poisoned indices path is not exist!".format(poisoned_indices_path))
    
    with open(poisoned_indices_path, 'rb') as f:
        poi_indeces = pickle.load(f)
    
    if clean:
        clean_indices = np.array(list(set(range(data_nums))-set(poi_indeces)))  
        target_set.x = target_set.x[clean_indices]
        target_set.y = np.array(target_set.y)[clean_indices]
    else:
        ...
        noise[poi_indeces] += raw_noise[poi_indeces]
...
def get_whole_loader(
        dataset, batch_size, root='./data', train=True,
        clean=None, noise_path=None, fitr=None):

    if dataset == 'imagenet' or dataset == 'imagenet-mini':
        return imagenet_utils.get_whole_loader(
            dataset=dataset, batch_size=batch_size, root=root, train=train, clean=clean, noise_path=noise_path, fitr=fitr
        )
    ...
    if not clean:
        ...
        assert raw_noise.dtype == np.int8
        raw_noise = raw_noise.astype(np.int16)
        raw_noise = np.transpose(raw_noise, [0,2,3,1])
        imgs = target_set.x.astype(np.int16) + raw_noise
        imgs = imgs.clip(0,255).astype(np.uint8)
        target_set.x = imgs
...
def get_arch(arch, dataset):
    ...
    if arch == 'resnet18':
        ...
        if dataset == 'cifar10' or dataset == 'cifar100':
            ...
            # return timm.create_model('swin_small_patch4_window7_224')
